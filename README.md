...
👉 https://poker-insight.vercel.app
...
## 5. FigmaMake を活用した UI デザイン生成

- FigmaMake を用いて、プロダクトの UI/UX を高速にプロトタイピング
- テキストプロンプトから画面ラフを生成し、そこから細部を手作業で調整
- 色・余白・階層構造をプロダクト全体で統一しつつ、モバイル前提のレイアウトを設計
- Tailwind CSS のユーティリティと相性が良い「コンポーネント単位のデザイン」を意識
  → 実装前に UI の方向性を固めることで、開発スピードと一貫性を向上
